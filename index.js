...
exports.vorpal.localStorage("bull-repl-default");
exports.localStorage = exports.vorpal.localStorage;
exports.LAST_SAVED_CONNECTION_NAME = "_last-active";
exports.vorpal
    ...
    const connOpts = { prefix };
    if (options.tls) {
        const tlsOpts = {};
        if (options.ca) {
            tlsOpts.ca = fs_1.readFileSync(options.ca);
        }
        connOpts.redis = { tls: tlsOpts };
    }
    await queue_1.setQueue(name, url, connOpts);
    exports.localStorage.setItem(exports.LAST_SAVED_CONNECTION_NAME, JSON.stringify({ name, url, prefix }));
    utils_1.logGreen(`Connected to ${url}, prefix: ${prefix}, queue: ${name}`);
    exports.vorpal.delimiter(`BULL-REPL | ${prefix}.${name}> `).show();
}));
exports.vorpal.command("connect-list", "list of saved connections").action(utils_1.wrapTryCatch(async () => {
    console.table(exports.localStorage._localStorage.keys);
}));
exports.vorpal.command("connect-rm <name>", "remove saved connection").action(utils_1.wrapTryCatch(async ({ name }) => {
    if (name === exports.LAST_SAVED_CONNECTION_NAME) {
        return utils_1.logYellow(`Can't use reserved name, please use another`);
    }
    const savedItem = exports.localStorage.getItem(name);
    if (savedItem) {
        exports.localStorage.removeItem(name);
        utils_1.logGreen(`Connection "${name}" removed`);
    }
    else {
        utils_1.logYellow(`Connection "${name}" not found`);
    }
}));
exports.vorpal.command("connect-save <name>", "save current connection").action(utils_1.wrapTryCatch(async ({ name: nameForSave }) => {
    if (nameForSave === exports.LAST_SAVED_CONNECTION_NAME) {
        return utils_1.logYellow(`Can't use reserved name, please use another`);
    }
    await queue_1.getQueue();
    const { name, url, prefix } = JSON.parse(exports.localStorage.getItem(exports.LAST_SAVED_CONNECTION_NAME));
    exports.localStorage.setItem(nameForSave, JSON.stringify({ name, url, prefix }));
    utils_1.logGreen(`Connection "${nameForSave}" saved`);
}));
exports.vorpal.command("connect-to <name>", "connect to saved connection").action(utils_1.wrapTryCatch(async ({ name: connectToName }) => {
    const savedItem = exports.localStorage.getItem(connectToName);
    if (!savedItem) {
        return utils_1.logYellow(`Connection "${connectToName}" not found`);
    }
    const { name, url, prefix } = JSON.parse(savedItem);
